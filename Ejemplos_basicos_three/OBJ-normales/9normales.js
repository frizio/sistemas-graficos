// File 9normals.js
 ...
 //Create the Three.js WebGL renderer
  ...
  var controls = new function(){
	  ...
	  this.cameray = 200;
	  this.cameraz = 400;
	  this.planoturn = false;
	  this.cuboturn = false;
	  this.wholeobjectturn = false;
      ...
      this.updatePlaneTurn = function (e) {
	    turnPlane = e;	
	  }
      this.updateCubeTurn = function (e) {
	    turnCube = e;	
	  }
	  this.updateSceneTurn = function (e) {
	    turnScene = e;	
	  }		  
    }
	...
	loader.load( 'exportwood.obj', 'exportwood.mtl', function ( object ) {
					object.traverse( function( child ) {
						if ( child instanceof THREE.Mesh ) {
						child.geometry.computeVertexNormals();
						}
					} );
					cubo = object.getObjectByName("Cube.001"); // If there is no such object, cubo will stay undefined
					plano = object.getObjectByName("Plane"); // If there is no such object, plano will stay undefined					
					wholeobject = object;
  ...
  f1.add(controls, 'lightx', -800,800).onChange(controls.updateLight1x);
  f1.add(controls, 'lighty', -800,800).onChange(controls.updateLight1y);
  f1.add(controls, 'lightz', -800,800).onChange(controls.updateLight1z);
  ...
  f2.add(controls, 'light3x', -800,800).onChange(controls.updateLight2x);
  f2.add(controls, 'light3y', -800,800).onChange(controls.updateLight2y);
  f2.add(controls, 'light3z', -800,800).onChange(controls.updateLight2z);
  ...
  f3.add(controls, 'light2x', -800,800).onChange(controls.updateLight3x);
  f3.add(controls, 'light2y', -800,800).onChange(controls.updateLight3y);
  f3.add(controls, 'light2z', -800,800).onChange(controls.updateLight3z);
  f3.addColor(controls, 'color3').onChange(controls.updateLight3color);
  f3.add(controls, 'visible3').onChange(controls.visibleLight3);
  
  var f4 = gui.addFolder('Turn');
  f4.add(controls, 'cuboturn').onChange(controls.updateCubeTurn);
  f4.add(controls, 'planoturn').onChange(controls.updatePlaneTurn);
  f4.add(controls, 'wholeobjectturn').onChange(controls.updateSceneTurn);
 ...
 var light = new THREE.DirectionalLight(controls.color1, 1);
  ...
  stats.update();
  if (turnPlane) plano.rotation.y += 0.02;
  if (turnCube) cubo.rotation.y -= 0.02;
  if (turnScene) wholeobject.rotation.x += 0.01;
