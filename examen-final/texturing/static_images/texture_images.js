...
function draw_scene() {
  ...
  var starmapUrls = ["starsmap.png", "starsmap.png", "starsmap.png", "starsmap.png", "starsmap.png", "starsmap.png"];
  var textureSkycube = THREE.ImageUtils.loadTextureCube( starmapUrls );
  ...
  var skycubeGeometry = new THREE.SphereGeometry(500, 32, 32);
  ...
  var geometry1 = new THREE.PlaneGeometry( 100, 100, 8, 8 );
  var geometry2 = new THREE.PlaneGeometry( 100, 100, 8, 8 );
  ...
  // Material
  var material1 = new THREE.MeshPhongMaterial( { color: 0xffffff, map: facesTextures[0] } );
  var material2 = new THREE.MeshPhongMaterial( { color: 0xffffff, map: facesTextures[1] } );
  ...
  // Auxiliar variables to calculate the card face not visible from the camera
  var count = 0;
  var angle = 0;
  var angle1 = 0;
  var angle2 = 0;
  ...
  function render() {
    ...
    /*
    The idea is to calculate the angles between
    the camera position (treated as vector) and
    normal vector to the surface of each 2 faces, obtaining two angle:
    1) angle between camera-face1; 2) angle between camera and face2
    ...
    */
    if (count % 200 == 0) {
      ...
      */
      ...
      dirToCamera = camera.position.clone();
      dirToCamera.normalize();

      normal1 = mesh1.geometry.faces[0].normal;
      normal2 = mesh2.geometry.faces[0].normal;
      angle1 = Math.acos(normal1.dot(dirToCamera)) * 180/Math.PI;
      angle2 = Math.acos(normal2.dot(dirToCamera)) * 180/Math.PI;

      console.log("plane position position", card.position);
      console.log("camera position", camera.position.x, camera.position.y, camera.position.z);

      console.log("face 1 normal", mesh1.geometry.faces[0].normal);
      console.log("face 2 normal", mesh2.geometry.faces[0].normal);
      console.log("camera position norm", dirToCamera);
      console.log("angle camera-face1", angle1);
      console.log("angle camera-face2", angle2);

      if (angle1 > angle2) {
        mesh2.material.map = facesTextures[Math.floor(Math.random() * 6)];
        mesh2.material.needsUpdate = true;
        console.log(count, "Change face 2");
      } else {
        mesh1.material.map = facesTextures[Math.floor(Math.random() * 6)];
        mesh1.material.needsUpdate = true;
        console.log(count, "Change face 1");
      }
