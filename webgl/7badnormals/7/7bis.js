// 7.js
// Based on the OBJViewer example of book WebGL Programming Guide
...
function main() {
  ...
  readShaderFile(gl, 'Ex7vshader-malo.vert', 'v');
  readShaderFile(gl, 'Ex7fshader.frag', 'f');
}

function goahead_shaders_readed(gl) {
  ...
  readOBJFile('piramide.obj', gl, model, 1, true);

  var currentAngle = 0.0;
  // Current rotation angle [degree]


  var tick = function() {   // Start drawing
    currentAngle = animate(currentAngle); // Update current rotation angle
    draw(gl, gl.program, currentAngle, viewProjMatrix, model);
    //if (!control_global) requestAnimationFrame(tick, canvas); // Only repeats while waiting load of model
...
function onReadShader(gl, fileString, shader) {
  ...
  // When both are available, continue with program
  if (VSHADER_SOURCE && FSHADER_SOURCE) goahead_shaders_readed(gl);
}
...
function draw(gl, program, angle, viewProjMatrix, model) {
  ...
  // Aggiungiamo una trasformazione per una matrice non ortogonale e l'effetto  non è corretto)
  ...
  //g_normalMatrix.setIdentity();

  // Initializiamo la normale con la matrice di odello
  g_normalMatrix.set(g_modelMatrix);
...
var ANGLE_STEP = 2;   // The increments of rotation angle (degrees)
